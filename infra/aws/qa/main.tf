...
resource "aws_key_pair" "bastion" {
  key_name   = "qa-bastion-key"
  public_key = file("C:/Users/niklaus/.ssh/qa-bastion.pub")
}
...
module "ec2_core" {
  ...
  ami_id              = data.aws_ami.amazon_linux.id
  instance_type       = "t3.micro"
  subnet_id           = module.subnets.private_core_subnet_id
...
module "ec2_data" {
  ...
  ami_id              = data.aws_ami.amazon_linux.id
  instance_type       = "t3.micro"
  subnet_id           = module.subnets.private_data_subnet_id
  security_group_ids  = [module.security_groups.data_sg_id]
  key_name            = aws_key_pair.bastion.key_name
  associate_public_ip = false
